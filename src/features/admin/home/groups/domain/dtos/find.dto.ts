...
export class FindGroupsDto {
    ...
    static create(obj: { [key: string]: any }): [string[] | string | undefined, FindGroupsDto?] {
        ...
        if (name) {
            if (typeof name !== 'string' || name.trim().length < 0) errorsPropertiesTypes.push("'name' must be a string and at least 4 characters long.");
        }
        ...
        return [undefined, new FindGroupsDto(attributes)]
    }
